...
def view_hocphan():
    ...
    # search_hocphan_button = Button(root, text="Tìm kiếm", command=search_diemhocphan, width=20)
...
def view_Diemhp_details(student_info):
    ...
    update_button.grid(row=8, column=1, padx=10, pady=10)
#end 
def view_danhsachp_details(student_info):
    # Tạo một cửa sổ mới
    details_window = Toplevel(root)
    details_window.title("Môn học:" + student_info[1]) 
...
def add_hocphan_window():
    ...
    # Combobox get mã lớp
    Label(add_window, text="Mã học:").grid(row=0, column=0, padx=10, pady=5)
    ma_mon_entry = Entry(add_window, width=40)
    ma_mon_entry.grid(row=0, column=1, padx=10, pady=5)

     # Combobox get mã sinh viên
    Label(add_window, text="Tên học phần:").grid(row=1, column=0, padx=10, pady=5)
    ten_mon_entry = Entry(add_window, width=40)
    ten_mon_entry.grid(row=1, column=1, padx=10, pady=5)
    Label(add_window, text="Số tín chỉ:").grid(row=2, column=0, padx=10, pady=5)
    sotinchi_entry = Entry(add_window, width=40)
    sotinchi_entry.grid(row=2, column=1, padx=10, pady=5)

    Label(add_window, text="Mã học kỳ:").grid(row=3, column=0, padx=10, pady=5)
    ma_hk_entry = Entry(add_window, width=40)
    ma_hk_entry.grid(row=3, column=1, padx=10, pady=5)

    # Thêm nút để thực hiện thêm điểm học phần
    add_button = Button(add_window, text="Thêm Học phần", command=lambda: add_hocphan(ma_mon_entry.get(), ten_mon_entry.get(), sotinchi_entry.get(), ma_hk_entry.get()), width=20)
...
def add_diemhocphan_window():
    ...
    # Thêm nút để thực hiện thêm điểm học phần
    add_button = Button(add_window, text="Thêm Điểm học phần", command=lambda: add_diemhocphan(ma_sv_combobox.get(), ma_mon_combobox.get(), diem_giua_ky_entry.get(), diem_thi_hp_entry.get()), width=20)
    add_button.grid(row=5, column=1, padx=10, pady=10)
